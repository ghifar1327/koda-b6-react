...
export default function Payment() {
    ...
    const deliveryFee =
      delivery === "Door Delivery" ? 10000 : 0;
    ...
    <div className="grid md:grid-cols-5 gap-10">
            ...
            return(
                ...
                <div key={item.id} className="bg-[#E8E8E84D] w-full p-2 pr-5 rounded flex justify-between items-center gap-2">
                    ...
                    <div className="flex flex-col justify-between">
                        ...
                        {item.quantity}pcs | {item.size} | {item.hotIce} | {delivery}
                        </p>
        ...
        <div className="font-normal">
            <Button onClick={handleSubmit} orange>Checkout</Button>
        </div>
      ...
      <section className="order-2 md:order-3 md:col-span-3 w-full flex flex-col gap-5">
        ...
        <Input label="Email" type="email" id="email" value={form.email} onChange={handleChange} defaultValue={isLogin.email} placeholder="Enter Your Email"
        >
          <Mail size={18} />
        </Input>
        <Input label="Full Name" type="text" id="fullname" value={form.fullName} onChange={handleChange} defaultValue={isLogin.fullName} placeholder="Enter Your Full Name">
          <User size={18} />
        </Input>
        <Input label="Address" type="text" id="address" value={form.address} onChange={handleChange} defaultValue={"karawang"} placeholder="Enter Your Address">
          <MapPin size={18} />
        </Input>
        <div className="flex flex-col gap-5">
          ...
          <div className="flex gap-7">
            <Input type="radio" name="delivery" value="Dine In" id="dineIn"  checked={delivery === "Dine In"} onChange={(e)=> setDelivery(e.target.value)}>
              Dine In
            </Input>
            <Input type="radio" name="delivery" value="Door Delivery" id="doorDelivery"  checked={delivery === "Door Delivery"} onChange={(e)=> setDelivery(e.target.value)}>
              Door Delivery
            </Input>
            <Input type="radio" name="delivery" value="Pick Up" id="pickUp" checked={delivery === "Pick Up"} onChange={(e)=> setDelivery(e.target.value)}>
              Pick Up
            </Input>
          </div>
        </div>
