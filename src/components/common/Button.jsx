...
export function Button({
  ...
  ...rest
    ...
    <button
      type={type}
      {...rest}
      className={`${orange && "bg-primary border border-primary hover:bg-white"} ${shadow && "shadow-md border border-[#DEDEDE]/10"} ${size ? size : "w-full p-2 gap-5"} ${border} ${radius ? radius : "rounded-md" } flex items-center justify-center  cursor-pointer`}
    >
